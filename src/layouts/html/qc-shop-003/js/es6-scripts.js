/*
 * Quick Commerce by Firebrand
 * Copyright 2017 Firebrand Solutions
 * ES6 custom scripts
 */
...
//------------------------------------------------------------------------------
let $scrollTop = $('.scroll-to-top-btn');
...
$scrollTo.on('click', function(event) {
    let $elemOffsetTop = $(this).data('offset-top');
...
// Toggle Submenu
//------------------------------------------------------------------------------
let $hasSubmenu = $('.menu-item-has-children > a');
...
//------------------------------------------------------------------------------
let $subscribeToggle = $('.subscribe-btn'),
...
// Shop Filters Toggle
//------------------------------------------------------------------------------
let filtersToggle = $('[data-toggle="filters"]'),
        filtersWrap = $('.filters'),
...
filtersToggle.on('click', function(e) {
    let currentFilter = $(this).attr('href');
    if($(this).is('.active')) {
        closeFilterPane();
    } else {
        closeFilterPane();
...
$('.color-select').on('change', function() {
    let target = $(this).find(':selected').data('image');
...
//------------------------------------------------------------------------------
let ajaxPostLink = $('.ajax-post-link'),
...
// Image Carousel
//------------------------------------------------------------------------------
let $imageCarousel = $( '.image-carousel .inner' );
if ( $imageCarousel.length > 0 ) {
    $imageCarousel.each( function () {

        let dataLoop 	 = $( this ).parent().data( 'loop' ),
...
// Hero Slider
//------------------------------------------------------------------------------
let $heroSlider = $( '.hero-slider .inner' );
...
// Video Popup
//------------------------------------------------------------------------------
let $videoBtn = $( '.video-popup-btn, .gallery-video, .video-popup-placeholder .play-btn' );
...
if($googleMap.length > 0) {
    $googleMap.each(function(){
        let mapHeight = $(this).data('height'),
